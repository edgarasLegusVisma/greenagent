...
export function generateSuggestions(tracker: GreenTracker): Suggestion[] {
  ...
  for (const g of planningGroups) {
    ...
    suggestions.push({
      severity: 'high',
      title: `Planner used ${g.toolRounds} tool rounds to explore the codebase (${g.totalTokens.toLocaleString()} tokens) — see docs/agents/planner.md`,
      detail:
  ...
  for (const g of readerGroups) {
    suggestions.push({
      severity: 'high',
      title: `Code Reader re-read files the Planner already summarised (${g.totalTokens.toLocaleString()} tokens) — see docs/agents/code-reader.md`,
      detail:
  ...
  for (const g of architectGroups) {
    suggestions.push({
      severity: 'medium',
      title: `Architect agent produced ${g.totalTokens.toLocaleString()} tokens of design that could be part of the implementation prompt — see docs/agents/architect.md`,
      detail:
  ...
  for (const g of rewriteGroups) {
    suggestions.push({
      severity: 'medium',
      title: `Backend Developer (fixes) rewrote files after every review (${g.totalTokens.toLocaleString()} tokens) — see docs/agents/backend-developer.md`,
      detail:
  ...
  if (distinctImplGroups.length >= 2) {
    ...
    suggestions.push({
      severity: 'medium',
      title: `${distinctImplGroups.length} separate implementation agents (${totalTokens.toLocaleString()} tokens combined) — see docs/agents/backend-developer.md`,
      detail:
  ...
  for (const g of validationGroups) {
    ...
    suggestions.push({
      severity: 'medium',
      title: `Quality Gate is redundant after Code Review (${g.totalTokens.toLocaleString()} tokens) — see docs/agents/quality-gate.md`,
      detail:
