...
export class XRay {
  ...
  static report(tracker: GreenTracker, title: string = 'Workflow X-Ray Report'): void {
    ...
    console.log(`  ${DIM}Useful work ratio: ${Math.round(usefulRatio * 100)}% | High-severity issues: ${highSev}${RESET}`);
    console.log();
    console.log(`${CYAN}${BOLD}${'═'.repeat(W)}${RESET}`);
  ...
  static reportToMarkdown(tracker: GreenTracker, title: string = 'X-Ray Report'): string {
    ...
    lines.push('');
    lines.push(`**${score}** (Useful work: ${Math.round(usefulRatio * 100)}% | High-severity issues: ${highSev})`);
    lines.push('');
